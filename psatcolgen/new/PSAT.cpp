...
//recover a column from the Sat Solver solution; sets a negative first element if the SAT instance was Unsatisfiable
...
//tries to solve the SAT instance
...
//begin iterations:
    ...
    //trying simplex first
        ...
        satIns->restart();
        satIns->restartSolver(solver);
        satIns->addGamma();
        for(i=0;i<count;i++) satIns->ineqtoCNF(inequalities[i]);
        SATsat = SATSolve(solver, 0);
        getColumn(columnSimplex, nProb+1, solver, SATsat);
