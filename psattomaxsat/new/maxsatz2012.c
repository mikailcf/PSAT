...
#include "maxsatz2012.h"

static FILE *file_redir;
...
#define debug_overflow(arrayIndex, maxSize) \
      ...
      if (arrayIndex > maxSize) {\
        fprintf(file_redir, "DEBUG: arrayIndex.\n"); \
        exit(0);\
      }\
   #endif
*/
...
void add_new_lit_in(int *pos, int clause) {
  ...
  if (LIT_IN_STACK_fill_pointer > tab_clause_size_XL - 5) {
    // fprintf(file_redir, "DEBUG: LIT_IN_STACK.\n");
    exit(0);
  }
#endif
...
int lire_clause(long int wght, int *lits, int length, int clause_number, int instance_type) {
    int j, jj, ii, /*length,*/ tautologie, /*lits[10000],*/ lit, lit1;
    lli_type weight;

    clause_number += BASE_NB_CLAUSE;
    ...
    if (tautologie == FALSE) {
        sat[clause_number]= (int *)malloc((length+1) * sizeof(int));

        for (j=0; j<length; j++) {
            if(lits[j] < 0) sat[clause_number][j] = abs(lits[j]) - 1 + NB_VAR ;
            else sat[clause_number][j] = lits[j]-1;
        }

        sat[clause_number][length] = NONE;
        clause_length[clause_number] = length;
        clause_weight[clause_number] = weight;

        if(partial == 0) HARD_WEIGHT += weight;

        clause_state[clause_number] = ACTIVE;
    }
    else return 1;  // a clausula em questao eh uma tautologia, entao deve ser ignorada
...
int solve(int clauses){
  // sscanf(pLine, "p %s %d %d %lli", word2, &NB_VAR, &NB_CLAUSE, &HARD_WEIGHT);
  // fprintf(file_redir, "c Instance info: p %s %d %d %lli\n",  word2, NB_VAR, NB_CLAUSE, HARD_WEIGHT);

  NB_CLAUSE = clauses + BASE_NB_CLAUSE;
  INIT_NB_CLAUSE = NB_CLAUSE;
...
void print_clause(int clause) {
  ...
  // fprintf(file_redir, "(%i, %i, %i) [%lli] ", clause, clause_length[clause],  clause_state[clause], clause_weight[clause]);
  vars_signs = var_sign[clause];
  for(var = *vars_signs; var != NONE; var = *(vars_signs += 2)) {
    // if (*(vars_signs + 1) == NEGATIVE)
      // fprintf(file_redir, "-");
    // fprintf(file_redir, "%i ", var + 1);
  }
  // fprintf(file_redir, "0");
}
...
int reduce_clauses(int var) {
  ...
  if (UNITCLAUSE_STACK_fill_pointer > tab_unitclause_size - 5) {
    // fprintf(file_redir, "DEBUG: UNITCLAUSE_STACK.\n");
    exit(0);
  }
#endif
  break;
    ...
    if (clause_state[clause] == ACTIVE) {
      ...
      switch (clause_length[clause]) {
        ...
        if (MY_UNITCLAUSE_STACK_fill_pointer > tab_unitclause_size - 5) {
          // fprintf(file_redir, "DEBUG: MY_UNITCLAUSE_STACK.\n");
          exit(0);
        }
#endif
        break;
...
void create_binaryclause(int var1, int sign1, 
 ...
 lli_type min_weight) {
  ...
  if (NB_CLAUSE > tab_clause_size - 5) {
    // fprintf(file_redir, "DEBUG: NB_CLAUSE.\n");
    exit(0);
  }
#endif
}
...
int verify_binary_clauses(int *varssigns, int var1, int sign1, 
  ...
  if (var1==*varssigns) {
    if ((*(varssigns+1)!=1-sign1) || (var2!=*(varssigns+2)) ||
      (*(varssigns+3)!=1-sign2)) {
      // fprintf(file_redir, "VBC problem..");
      return FALSE;
  }
}
else {
...
int create_complementary_binclause(int clause, int clause1, 
 int clause2, lli_type min_weight) {
  ...
  //   fprintf(file_redir, "CCB problem...");
#endif
...
int get_satisfied_literal(int clause) {
  ...
  // fprintf(file_redir, "ERROR: Satisfied literal not found.\n");
  return NONE;
}
...
int linear_conflict(int clause,  lli_type *min_weight) {
  ...
  if (i>4)
    return FALSE;
  if (i==0) {
    // fprintf(file_redir, "ERROR: Conflict without reason.\n");
    return FALSE;
  } else {
...
void reduce_clause_weight(int clause, int weight) {
  if (clause_weight[clause] < UB) {
    ...
    if (SAVED_WEIGHTS_CLAUSE_fill_pointer > tab_clause_size - 5) {
      // fprintf(file_redir, "DEBUG: SAVED_WEIGHTS_CLAUSE.\n");	
      exit(0);
    }
#endif
...
void remove_linear_reason(int clause, lli_type min_weight) {
  ...
  // else
    // fprintf(file_redir, "ERROR: Negative weight.\n");
  ...
  if (CLAUSES_WEIGHTS_TO_REMOVE_fill_pointer > tab_clause_size - 5) {
    // fprintf(file_redir, "DEBUG: CLAUSES_WEIGHTS_TO_REMOVE.\n");
    exit(0);
  }
#endif
}
  ...
  if ((clause=assign_and_unitclause_process(var, value, 
   saved_unitclause_stack_fill_pointer))!=NO_CONFLICT) {
    if (cycle_conflict(clause, var)==TRUE) { 
      CMTR[value]++; //fprintf(file_redir, "sdf...");
...
void create_unitclause(int lit, int subsumedclause, 
 int p_clause, lli_type weight) {
  ...
  // if (flag==FALSE)
  //   fprintf(file_redir, "ERROR: subsumed clause not found.\n");
  NB_CLAUSE++;
#ifdef DEBUG
  if (NB_CLAUSE > tab_clause_size - 5) {
    // fprintf(file_redir, "DEBUG: NB_CLAUSE.\n");
...
int assign_value(int var, int current_value, int rest_value) {
  ...
  //   fprintf(file_redir, "ERROR: Assigning passive variable.\n");
  var_state[var] = PASSIVE;
  push(var, VARIABLE_STACK);
  ...
  for (var = *vars; var != NONE; var = *(vars += 2), vars_signs += 2) {
    ...
    *(vars_signs + 1) = sen;
    ...
    if (nb_undo_learned[var] > max_var_learned - 5) {
      // fprintf(file_redir, "DEBUG: nb_undo_learned[var].\n");
      exit(0);
    }
#endif
  }
...
void dpl() {
  ...
  do {
    if (VARIABLE_STACK_fill_pointer==NB_VAR) {
      UB = NB_EMPTY;
      //   if (UB==201)
      //  fprintf(file_redir, "sdf");
      nb = verify_solution();
      // if (nb != NB_EMPTY)
      //   fprintf(file_redir, "ERROR: Solution verification fails, real_empty = %lli, NB_EMPTY = %lli.\n",  nb, NB_EMPTY);
      // fprintf(file_redir, "o %lli\n", UB);
      for (i = 0; i < NB_VAR; i++)
...
void ubcsat(char file[]) {
  ...
  if (ls == (FILE*) NULL) {
    // fprintf(file_redir, "WARNING: Ubcsat not found.\n");
    return;
  }
  ...
  ls = fopen(fout, "r");
  if (ls == (FILE*) NULL) {
    // fprintf(file_redir, "WARNING: Ubcsat output file not found.\n");
    return;
  }
  ...
  // if (i != NB_VAR) {
  //   fprintf(file_redir, "WARNING: Ubcsat problem.\n");
  // }
}
...
int maxsatz_main(int argc, char *argv[], int solution[], long long int *opt_solution, int *sat, int size){
  ...
  file_redir = fopen("saidamax.out", "w");
  
  if (argc <= 1) {
    // fprintf(file_redir, "Using format: %s input_instance [-l]\n\t-l: without local search.", argv[0]);
    return FALSE;
  }
  ...
  // fprintf(file_redir, "c ----------------------------\n");
  // fprintf(file_redir, "c - Weighted Partial MaxSATZ -\n");
  // fprintf(file_redir, "c ----------------------------\n");
#ifdef DEBUG
  // fprintf(file_redir, "c DEBUG mode ON\n");
#endif
  
  switch (build_simple_sat_instance(argv[1])) {
    case FALSE:
    // fprintf(file_redir, "ERROR: Input file error\n");
    return FALSE;
    case TRUE:
    UB = HARD_WEIGHT;
    if (argc < 3 || strcmp(argv[2], "-l") != 0)
      ubcsat(argv[1]);
    // else
      // fprintf(file_redir, "c Without local search.\n");
    // fprintf(file_redir, "o %lli\n", UB);
    if (UB != 0) {
      init();
      dpl();
    ...
    break;
    case NONE:
    // fprintf(file_redir, "An empty resolvant is found!\n");	
    break;
  }
  mess=times(a_tms); endtime = a_tms->tms_utime;
  
  // fprintf(file_redir, "c Learned clauses = %i\n", INIT_BASE_NB_CLAUSE - BASE_NB_CLAUSE);
  // fprintf(file_redir, "c NB_MONO= %lli, NB_BRANCHE= %lli, NB_BACK= %lli \n",  NB_MONO, NB_BRANCHE, NB_BACK);
  if (UB >= HARD_WEIGHT) {
    // fprintf(file_redir, "s UNSATISFIABLE\n");
    *sat = 0;
  } else {
    // fprintf(file_redir, "s OPTIMUM FOUND\nc Optimal Solution = %lli\n", UB);
    // fprintf(file_redir, "v");
    *opt_solution = UB;
    for (i = 0; i < NB_VAR; i++) {
      if (var_best_value[i] == FALSE){
        // fprintf(file_redir, " -%i", i + 1);
        if(i < size){
          solution[i] = -(i+1);
        }
      }
  ...
  // fprintf(file_redir, "Program terminated in %5.3f seconds.\n", ((double)(endtime-begintime)/CLOCKS_PER_SEC));

  fp_time = fopen("resulttable", "a");
  // fprintf(fp_time, "wpmsz-2.5 %s %5.3f %lld %lld %lld %d %d %d %d\n",  saved_input_file, ((double)(endtime-begintime)/CLOCKS_PER_SEC),  NB_BRANCHE, NB_BACK,   UB, NB_VAR, INIT_NB_CLAUSE, NB_CLAUSE-INIT_NB_CLAUSE, CMTR[0]+CMTR[1]);
  // fprintf(file_redir, "wpmsz-2.5 %s %5.3f %lld %lld %lld %d %d %d %d\n",  saved_input_file, ((double)(endtime-begintime)/CLOCKS_PER_SEC),  NB_BRANCHE, NB_BACK, UB, NB_VAR, INIT_NB_CLAUSE, NB_CLAUSE-INIT_NB_CLAUSE, CMTR[0]+CMTR[1]);
  fclose(fp_time);
