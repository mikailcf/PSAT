...
void getColumn(int *column, int size, long limit, int *solution, long long int *opt_solution, int *sat){
    int i;
...
void getColumn(int *column, int size, Solver *solver, int sat){
    int i,lit;
    ...
    column[0] = 1;
    for(i = 1; i <= size - 1; i++){
        lit = (solver->model[i-1]==l_True) ? i : -i;
...
void checkConsistency(int* notinGamma,int size){
    ...
    ClauseGen* satInst;
    satInst = new ClauseGen(solver);
    cons = new int[size];
    for(i=0;i<size;i++){
        satInst->restart_();
        satInst->addGamma();  
        for(j=1;j<size;j++){
            if(j<=i)satInst->andN_(1,j);
            else satInst->andN_(1,-j);
        }
        sat = SATSolve(solver, 0);

        getColumn(cons, size, solver, sat);
...
void PSAT::solve(){
    ...
    maxsat_opt_solution = (long long int *) malloc(sizeof(long long int));
    maxsat_sat = (int *) malloc(sizeof(int));
...
//begin iterations:
    ...
    basis->setDet(1.0/inverse->getDet());
...
//  inverse->print();   
    ...
    //trying simplex, if could not substitute an inconsistent column
        ...
        satIns->restart_();
        setHARD_WEIGHT(top);
        clauses = satIns->addGamma(top, clauses);
        for(i = 0; i < count; i++) clauses = satIns->ineqtoWCNF_(inequalities[i], signal, clauses);

        maxsatz_main_new(clauses, satIns->gettopAtom()-1, maxsat_solution, maxsat_opt_solution, maxsat_sat, nProb+1);

        getColumn(newColumn, nProb+1, top-1+inequalities[0]->get(0), maxsat_solution, maxsat_opt_solution, maxsat_sat);
