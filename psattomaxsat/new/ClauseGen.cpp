...
void ClauseGen::restart_(){
    topAtom = 1;
    countClause = 0;
    varGamma = -1;
}

int ClauseGen::ineqtoWCNF_(Inequality *ineq, int *signal, int clauses){
    int size, i, lire;
    long sum = 0;
    size = ineq->getSize()-1;
    ...
    if(sum < -ineq->get(0)){//unsatisfiable inequality 
        clauses = or2_(sum+1, 1, clauses);
        clauses = or2_(sum+1, -1, clauses);
        return clauses;
    }
    for(i = 1; i <= size; i++) clauses = or2_(ineq->get(i), i*signal[i], clauses);  
    return clauses;
}

int ClauseGen::addGamma(long top, int clauses){
    int i, nClause, nVar, len = 0, lits[10000], lire;
    long lit;
    FILE * gamma;
    ...
    for(i = 0; i < nClause; i++){             //guarda gamma em um txt
        ...
        lits[len] = 0;
        lire = lire_clause(top, lits, len, clauses, 1);
        if(lire == 0) clauses++;  // clausula nao foi uma tautologia, entao nao ha porque a ignorar
    }
...
int ClauseGen::or2_( long a,long b, int clauses){ return orN_(3, clauses, a, b);}

int ClauseGen::orN_( int n, ...){
    int lits[10000], lire;
    long int weight;
    int clauses;
    int j;
    ...
    clauses = va_arg(args, int); 
    weight = va_arg(args, long);

    for(j = 0; j < n; j++) lits[j] = (int) va_arg(args, long);
    lits[j] = 0;
    lire = lire_clause(weight, lits, n-2, clauses, 1);
